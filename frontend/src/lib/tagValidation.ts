// Client-side required-field validation for garment tags. These rules mirror the
// backend `TagRequest` constraints exactly (non-blank `category`, `formality` 1–5,
// `warmth` 1–3) so the UI can gate save and avoid a round-trip 400. This is a UX
// guardrail, not a security boundary — the backend remains the source of truth.

/** The three required fields, as held in the editable form (unset = `null`). */
...
/** Field-keyed messages; an empty object means the required fields are valid. */
...
/** Returns a message per invalid required field; `{}` when all are valid. */
