...
- [ ] 5.1 In `frontend/src/index.css`, generalize the shell: remove the `#root:has(.stylist-layout) { max-width: 72rem }` exception and change `#root` from `max-width: 30rem` to `max-width: 72rem` (keep `margin: 0 auto`, `min-height: 100dvh`), so desktop-width fill is the default for every screen.
- [ ] 5.2 Constrain naturally-narrow screens so they **center** instead of stretching: give the Add-item form and Item-detail containers a comfortable `max-width` + `margin-inline: auto` (matching today's ~30rem measure) within the now-wider shell. Confirm the width-filling screens (`/assemble`, `/wardrobe`, `/saved`) use the full column.
- [ ] 5.3 Manually verify on a desktop viewport (`/`, `/assemble`, `/wardrobe`, `/add`, `/item/:id`, `/saved` fill/center correctly) and a ~390px phone viewport (all single-column, no horizontal scroll), and that the Stylist two-pane layout is visually unchanged; capture the screenshot proof artifacts.
- [ ] 5.4 Run `cd frontend && npm test -- --run` and the lint/format gate to confirm the CSS change regresses no existing frontend test.
