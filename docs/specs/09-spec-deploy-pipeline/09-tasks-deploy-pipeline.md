...
### [x] 4.0 Terraform IAM roles: instance role + CI OIDC role, boundary-capped + Access Analyzer clean (Unit 2c — validated IaC)
...
- [x] 4.1 In `iam.tf` declare the App Runner **instance role** `aws_iam_role` (`abreiss-ensemble-instance`) with `permissions_boundary` = the computed `abreiss-ensemble-boundary` policy ARN (account id + prefix, like bootstrap) and a trust policy for `tasks.apprunner.amazonaws.com`.
- [x] 4.2 Attach a least-privilege policy to the instance role: S3 `GetObject`/`PutObject`/`DeleteObject` on `arn:...:abreiss-ensemble-photos/*` (+ `ListBucket` on the bucket ARN if needed), DynamoDB `GetItem`/`PutItem`/`UpdateItem`/`DeleteItem`/`Query`/`Scan` on the table ARN, and `secretsmanager:GetSecretValue` on the three secret ARNs — all prefix-scoped. (No `ListBucket`: `S3PhotoStorage`, Unit 1, only ever calls `PutObject`/`GetObject`/`DeleteObject` by key — never `ListObjects` — so granting it would be an unused permission.)
- [x] 4.3 If App Runner requires a private-ECR **access role**, declare `abreiss-ensemble-ecr-access` (boundary-attached, trust `build.apprunner.amazonaws.com`) with a scoped ECR-pull policy (or `AWSAppRunnerServicePolicyForECRAccess`) and wire it into the service `authentication_configuration` (3.3). (Required: `aws_ecr_repository.app` is a private repo, so App Runner's `authentication_configuration.access_role_arn` is mandatory for image pull; wired alongside `instance_role_arn` in `apprunner.tf`.)
- [x] 4.4 Declare the GitHub OIDC **CI role** `abreiss-ensemble-ci` (boundary-attached): trust policy federating the pre-existing OIDC provider (`data` lookup — #16 allows `GetOpenIDConnectProvider`) with a `:sub` condition scoped to this repo + branch/ref; permissions exactly ECR push (`GetAuthorizationToken` + repo push/pull on the ECR ARN), `apprunner:StartDeployment`/`DescribeService`/`UpdateService` on the service ARN, and one `iam:PassRole` scoped to `abreiss-ensemble-*` for App Runner — no widening beyond #16.
- [x] 4.5 Render the instance-role and CI-role policy JSON to committed review copies under `terraform/deploy/policies/` (mirrors `terraform/bootstrap/policies/`); run `aws accessanalyzer validate-policy` on each and confirm no `ERROR`/`SECURITY_WARNING` (or document each). Capture the JSON + lint output as proof. (JSON rendered from a genuine live `terraform plan` under the scoped identity — see `09-task-04-proofs.md`; the lint call itself hit `AccessDeniedException` on `access-analyzer:ValidatePolicy`, a permission the scoped identity was never granted — documented as a known gap in `docs/AWS_ACCESS.md` and `terraform/deploy/policies/README.md`, with the exact commands for an operator with a broader session to complete the lint.)
- [x] 4.6 Add `outputs.tf` exposing the App Runner service URL, ECR repo URL, table + bucket names, and CI role ARN; ensure no secrets/account-id are output; `fmt`/`validate` clean.

### [x] 5.0 CI pipeline: OIDC build → ECR → App Runner deploy, plus standing checks (Unit 3 — validated CI)
...
- [x] 5.1 Add `.github/workflows/deploy.yml` on `push: branches: [main]` with `permissions: { id-token: write, contents: read }`; first step `aws-actions/configure-aws-credentials` assuming the CI role via `role-to-assume` (OIDC, no static keys), region `us-east-1`.
- [x] 5.2 Build + push: `docker build` the existing multi-stage `Dockerfile`, tag with the immutable git SHA (`${{ github.sha }}`), and push to the ECR repo URL (never `latest`-only). (ECR login implemented as `aws-actions/amazon-ecr-login@v2` rather than a raw `aws ecr get-login-password | docker login` shell pipe -- same OIDC creds, same effect, the standard GitHub Actions idiom for this step.)
- [x] 5.3 Trigger + verify rollout via `aws apprunner update-service --service-arn ...` (not `start-deployment`), then poll `describe-service` until `RUNNING`, failing the job on any other terminal status or a timeout. (Deviation from the task's literal wording, required by `apprunner.tf`'s already-shipped design: `image_identifier` is pinned to a literal `:latest` that ECR's IMMUTABLE tag policy forbids ever re-pushing, so each deploy must repoint the service at a new `sha-<git-sha>` tag -- `start-deployment` only redeploys the currently-configured tag and cannot pick up a new one; `update-service` both retags and starts the deployment. Documented in `apprunner.tf`'s own comment and in `deploy.yml`.)
...
### [x] 6.0 Live deploy + golden-path verification + deploy runbook (Unit 4 — operator-run)
...
- [x] 6.3 Operator: push to `main`; confirm the pipeline builds/pushes the SHA-tagged image and triggers the App Runner deployment; capture a redacted CI run log; `curl https://<url>/api/health` → `200`. (Run `29792951440` @ `00eff1f`: OIDC assume → SHA-tagged ECR push → `update-service` → `RUNNING`; health `200`. Surfaced + fixed: unset repo variables, and the enterprise's ID-stamped OIDC subject that the CI-role trust policy had to learn to match — see `09-proofs/09-task-06-proofs.md`.)
- [x] 6.4 Operator: run the golden path on the public URL from a browser/iPhone (add garment → Haiku tags → vibe → grounded outfit rendering own S3 photos → pushback re-pick); capture screenshots; verify an S3 object listing + a DynamoDB item read (redacted). (API-level golden path + S3 listing + DynamoDB read all verified live, plus the operator-captured browser screenshot of the stylist flow — see `09-proofs/09-task-06-proofs.md`.)
- [x] 6.5 Operator: confirm the deferred #16 gate — `apply` succeeds under only the scoped identity; if a permission gap is hit, close it as a narrowly-scoped `abreiss-ensemble-*` addition to the #16 policy (never `*`; a second managed policy if the size limit is hit) and note it in `docs/AWS_ACCESS.md`. (Satisfied during 6.1's live run: final apply + converging plan under only the scoped user, diagnostic policy deleted first; the two gaps were closed exactly per this rule — the `abreiss-ensemble-terraform-ext` second managed policy and the PassRole-condition removal, both in `docs/AWS_ACCESS.md`. Gate write-up: `09-proofs/09-task-06-proofs.md`.)
- [x] 6.6 Write the deploy runbook: README/`docs/DEVELOPMENT.md` steps (state-bucket bootstrap → apply → secret population → push-to-deploy → reading the URL → rollback by redeploying an earlier SHA tag) plus the architecture/security narrative in `docs/ARCHITECTURE.md`, cross-referenced in the `docs/AWS_ACCESS.md` style. (README "Deploy to AWS (App Runner)" runbook; `docs/DEVELOPMENT.md` "Deploying (operator-run)"; `docs/ARCHITECTURE.md` "Deployment (shipped — issue #9)" + CI-auth security bullet.)
