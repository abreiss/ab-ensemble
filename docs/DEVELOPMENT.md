...
- **Accounts (issue #14):** Ensemble uses email/password accounts. Log in with `POST /api/auth` (`{email, password}`); sign up invite-only with `POST /api/accounts` (`{email, password, passcode}`), where `ENSEMBLE_PASSCODE` in the same `.env` is the shared **signup/invite code** (a blank value closes signup but existing accounts still log in). To get a first login without an invite code, set `ENSEMBLE_SEED_EMAIL` / `ENSEMBLE_SEED_PASSWORD` in `.env` and a default account is seeded idempotently on startup. Everything under `/api/**` other than `POST /api/auth`, `POST /api/accounts`, and `GET /api/health` requires the returned session token, sent as the `X-Ensemble-Session` header (or `?token=` for `<img>` GETs). See "Passcode gate & daily call cap" in `README.md`.
