...
class DynamoDbConfigTest {
	...
	void blankEndpoint_usesDefaultCredentialsWithNoOverride() {
		DynamoDbProperties props = new DynamoDbProperties("", "us-east-1", "ensemble-items", false);
	...
	void absentEndpoint_usesDefaultCredentialsWithNoOverride() {
		DynamoDbProperties props = new DynamoDbProperties(null, "us-east-1", "ensemble-items", false);
	...
	void presentEndpoint_overridesEndpointWithStaticDummyCredentials() {
		DynamoDbProperties props =
			new DynamoDbProperties("http://localhost:8000", "us-east-1", "ensemble-items", false);
